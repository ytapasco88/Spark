...
if [[ ! -e /opt/spark/spark-3.2.3-bin-hadoop3.2.tgz ]]; then
    ...
    sudo wget https://dlcdn.apache.org/spark/spark-3.2.3/spark-3.2.3-bin-hadoop3.2.tgz -P /opt/spark/
    sudo tar xvf /opt/spark/spark-3.2.3-bin-hadoop3.2.tgz  -C /opt/spark/
    if ! grep -R "export PYSPARK_PYTHON=/usr/bin/python3" ~/.bashrc 
    then
        echo "añadiendo variables de entornos no existentes"
        echo 'export SPARK_HOME=/opt/spark/spark-3.2.3-bin-hadoop3.2' >> ~/.bashrc
...
while true; do
    ...
    case $yn in
        [Yy]* ) sudo rm -r /opt/spark/spark-3.2.3-bin-hadoop3.2.tgz
                sudo wget https://dlcdn.apache.org/spark/spark-3.2.3/spark-3.2.3-bin-hadoop3.2.tgz -P /opt/spark/
                sudo tar xvf /opt/spark/spark-3.2.3-bin-hadoop3.2.tgz -C /opt/spark/
                if ! grep -R "export PYSPARK_PYTHON=/usr/bin/python3" ~/.bashrc
                then
